...
export async function remoeveUserFromGroup(user_email, admin_email){
    try {
        if(!user_email || !admin_email) throw new Error("No given data")
        if(user_email === admin_email) throw new Error("Admin cannot remove themselves from the group")
        const supabaseAdmin = await getAdmin()
        const adminData = await getUser(admin_email)
        if(!adminData.success) throw new Error(adminData.error)
        if(!adminData.data.admin) throw new Error("No access to remove user from group")
        ...
        if(userData.group_id !== adminData.data.group_id) throw new Error("User is not in your group")
