...
export async function requestsConsent(user_email, admin_email, agree){
    try{
        if(!user_email || !admin_email || typeof agree !== 'boolean') throw new Error("No given data")
        const subAccess = await subscriptionAccess(admin_email)
        if(!subAccess.success) throw new Error("Subscription access check failed: " + subAccess.error)
        if(!subAccess.access) throw new Error("Subscription expired. Please contact admin to renew subscription.")
...
export async function deleteConsent(userEmail, adminEmail) {
    try {
        if (!userEmail || !adminEmail) throw new Error("User email and admin email are required");
...
export async function removeUserFromGroup(user_email, admin_email){
    try {
        if (!user_email || !admin_email) throw new Error("User email and admin email are required");
        const subAccess = await subscriptionAccess(admin_email)
        if(!subAccess.success) throw new Error("Subscription access check failed: " + subAccess.error)
        if(!subAccess.access) throw new Error("Subscription expired. Please contact admin to renew subscription.")

        if(!user_email || !admin_email) throw new Error("User email and admin email are required")
        if(user_email === admin_email) throw new Error("Admin cannot remove themselves from the group")
        const supabaseAdmin = await getAdmin()
        ...
        if (!adminData.admin) throw new Error("No permission to remove users from group");
        if (!adminData.group_id) throw new Error("Admin is not in any group");
        ...
        if(userError) throw userError
        if(!userData) throw new Error("User not found")
        if(!userData.group_id) throw new Error("User is not in any group")
        if(userData.group_id !== adminData.group_id) throw new Error("User is not in your group")

        const updates = [
            // Reassign dates to admin
            supabaseAdmin
                .from('dates')
                .update({ assigned_to: adminData.user_id })
                .eq('assigned_to', userData.user_id),
                
            // Reassign contracts to admin  
            supabaseAdmin
                .from('contracts')
                .update({ uploaded_by: adminData.user_id })
                .eq('uploaded_by', userData.user_id),
                
            // Remove from trusted_users
            supabaseAdmin
                .from('trusted_users')
                .delete()
                .eq('user_id', userData.user_id),
            ...
            supabaseAdmin
                .from('users')
                .update({ group_id: null, admin: false })
                .eq('user_id', userData.user_id)
        ];
