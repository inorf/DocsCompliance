...
export async function createDate(dateData) {
    try{
        ...
        .from('dates')
        .insert([{group_id:groupResult.data.group_id, date_title:date_title, date_details:date_details, due_date:due_date, deadline_days:deadline_days, assigned_to:userData.user_id}])
...
export async function dateStatus(date_id, due_date=null, deadline_days=null) {
    try {
        ...
        if(due_date && deadline_days){
            const result = await calculateStatus(due_date, deadline_days)
            status = result.success ? result.data : status
        } else{
            const result = await calculateStatus(dateData.due_date, dateData.deadline_days)
            status = result.success ? result.data : status
...
export async function calculateStatus(due_date, deadline_days) {
    try {
...
export async function access(email, date_id) {
    try {
        const { data: userData, error: userError } = await supabase
        .from('users')
        .select('user_id, admin, group_id')
