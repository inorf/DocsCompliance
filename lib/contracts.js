...
export async function uploadFile(file, userEmail) {
    try {
        const subAccess = await subscriptionAccess(userEmail)
...
export async function deleteContract(cont_id, email) {
    try {
        ...
        if(!userCheck.success || !userCheck.data) throw new Error("User not found")
        ...
        if(contract.uploaded_by !== userCheck.data.user_id && !userCheck.data.admin) throw new Error("Unauthorized to delete this contract")
...
export async function deleteConDates(cont_id) {
    try {
        const supabaseAdmin = await getAdmin()
        const {data:dateLinks, error:linkError} = await supabaseAdmin
        .from('contracts_dates')
        .select('date_id')
        .eq('cont_id', cont_id)
        if(linkError) throw linkError
        const dateIds = dateLinks.map(link => link.date_id)
        const {error: deleteLinksError } = await supabaseAdmin
        .from('contracts_dates')
        .delete()
        .eq('cont_id', cont_id)
        if(deleteLinksError) throw deleteLinksError
        const {error: deleteDatesError } = await supabaseAdmin
        .from('dates')
        .delete()
        .in('date_id', dateIds)
...
export async function getFileData(cont_id) {
    try {
        const supabaseAdmin = await getAdmin()
        ...
        if (!contract) throw new Error("Contract not found")
        if (!contract.file_path) throw new Error("No file path found for contract")
        ...
        if(!downloadData) throw new Error("Failed to download file from storage")

        return{success:true, data:downloadData, error: null}
    } catch (error) {
        return{success:false, error: error.message}
    }
}